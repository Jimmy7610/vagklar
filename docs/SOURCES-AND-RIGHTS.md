...
Teoriboken används som **källmaterial**, inte som innehåll. Konkret:
...
- Ingen brödtext, ingen bild och ingen fråga ur boken återges.
- Allt innehåll i Vägklar är omskrivet från grunden med egna ord, egna exempel
  och egen grafik.
...
1. **Git.** `.gitignore` ignorerar `references/*.pdf`, `references/*.epub` och
   `references/**/extracted/`. Verifierat med `git check-ignore -v`.
...
referenssträng i en bundle — bygget föll i båda fallen.
